...
static const uint32_t BUFFER_SIZE = 512 * 1024;
...
void setup() {
  ...
  // HWCDC (USB-Serial-JTAG) initialization
  // Force USB re-enumeration for reliable Windows host detection
  Serial.end();
  delay(100);

  // MUST set RX buffer size BEFORE begin()
  // Default is only 256 bytes - too small for large JPEG frames at high baud
  Serial.setRxBufferSize(128 * 1024);

  Serial.begin(1500000);
  Serial.setTimeout(2000); // 2s timeout for large frames
